...
def get_pylon_articles(kb_id, pylon_api_key):
    ...
    log_and_print(f"Retrieved {len(processed_articles)} articles from Pylon")
    return processed_articles

def get_ada_articles(source_id, ada_api_key, ada_bot_url):
    ...
    log_and_print(f"Retrieved {len(processed_articles)} articles from Ada")
    return processed_articles

def bulk_upsert_articles(articles, source_id, ada_api_key, ada_bot_url):
    ...
    try:
        ...
        log_and_print(f"Bulk upserted {len(articles)} articles: {', '.join(article_titles[:3])}{'...' if len(articles) > 3 else ''}")
    except Exception:
        print("Ada response:", res.text)
        raise

def delete_articles_from_ada(article_ids, ada_api_key, ada_bot_url):
    ...
    try:
        res.raise_for_status()
        log_and_print(f"Deleted {len(article_ids)} articles: {', '.join(article_ids[:3])}{'...' if len(article_ids) > 3 else ''}")
    except Exception:
        print("Ada response:", res.text)
        raise

def perform_delta_sync(kb_id, source_id, pylon_api_key, ada_api_key, ada_bot_url):
    """Perform delta comparison and sync between Pylon and Ada."""
    log_and_print("Starting delta sync...")

    # Fetch articles from both systems
    pylon_articles = get_pylon_articles(kb_id, pylon_api_key)
    ada_articles = get_ada_articles(source_id, ada_api_key, ada_bot_url)
    ...
    log_and_print(f"Articles to create: {len(to_create)}")
    ...
    for article_id in pylon_ids & ada_ids:
        ...
        if pylon_hash != ada_hash:
            ...
            log_and_print(f"Content changed for '{pylon_articles[article_id]['title']}' (ID: {article_id})")
            log_and_print(f"  Pylon hash: {pylon_hash}")
            log_and_print(f"  Ada hash: {ada_hash}")

    log_and_print(f"Articles to update: {len(to_update)}")
    ...
    if articles_to_upsert:
        bulk_upsert_articles(articles_to_upsert, source_id, ada_api_key, ada_bot_url)

    # Articles in Ada not in Pylon → DELETE
    to_delete = list(ada_ids - pylon_ids)
    log_and_print(f"Articles to delete: {len(to_delete)}")

    if to_delete:
        delete_articles_from_ada(to_delete, ada_api_key, ada_bot_url)

    log_and_print(f"Delta sync completed: {len(to_create)} created, {len(to_update)} updated, {len(to_delete)} deleted")
